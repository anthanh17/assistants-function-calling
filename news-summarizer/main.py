...
def get_news(topic):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Acess all the fiels == loop through
            status = data["status"]
...
class AssistantManager:
    ...
    def __init__(self, model: str = model) -> None:
        ...
        # Retrieve existing assistant and thread if IDs are already
    ...
    def get_outputs_for_tool_call(self, tool_cal):
        # func_name = tool_cal.function.name
        topic = json.loads(tool_cal.function.arguments)["topic"]
        data_news = get_news(topic)
        final_str = ""
        ...
        if self.thread and self.run:
            while True:
                ...
                if run_status.status == "completed":
                    ...
                    tool_outputs = map(self.get_outputs_for_tool_call, tool_calls)
    ...
    with st.form(key="user_input_form"):
        ...
        if submit_button:
            ...
            manager.create_assistant(
                ...
                instructions="You are a personal article summarizer Assistant who knows how to take a list of article's titles and descriptions and the write a short summary of all the news articles",
                tools=[
                    {
                        "type": "function",
                        "function": {
